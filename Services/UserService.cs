...
namespace CafeRestaurant.Services
{
    public class UserService 
    {
        ...
        public UserService(CafeRestaurantEntities db)
        {
            this.db = db;
        ...
        public async Task<List<UserListItem>> UserSameRoleListAsync(int userRoleId)
        {
            ...
            {
                var list = await (from i in context.USER
        ...
        public async Task<List<UserDTO>> GetAllWithRoleNameAsync()
        {
            using (var context = new CafeRestaurantEntities())
            {
                var result = await context.Database.SqlQuery<UserDTO>("EXEC sp_UsersWithRoleName").ToListAsync();
                return result;
            }
        }
